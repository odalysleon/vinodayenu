...
class CheckoutPaymentStepCore extends AbstractCheckoutStep
{
    ...
    public function __construct( Context $context, TranslatorInterface $translator, PaymentOptionsFinder $paymentOptionsFinder, ConditionsToApproveFinder $conditionsToApproveFinder )
    {
    ...
    public function handleRequest( array $requestParams = array() )
    {
        ...
        $this->setTitle( $this->getTranslator()->trans( 'Payment', array(), 'Shop.Theme.Checkout' ) );
    }
...
//        if ( !empty( $products ) ) {
//            $cartegory = $this->getCategoryLevel2( new Product( $products[ 0 ][ 'id_product' ] ) );
//            if ( $cartegory == 'aceites' ) {
//                $isFree = true;
//                $action = $this->context->link->getModuleLink( 'pickeo', 'oilOrder' );
//            }
//        }
        ...
        $assignedVars = array( 'is_free' => $isFree, 'payment_options' => $paymentOptions, 'conditions_to_approve' => $conditionsToApprove, 'selected_payment_option' => $this->selected_payment_option, 'selected_delivery_option' => $selectedDeliveryOption, 'show_final_summary' => Configuration::get( 'PS_FINAL_SUMMARY_ENABLED' ), );
        if ( isset( $action ) )
            $assignedVars[ 'action' ] = $action;

        return $this->renderTemplate( $this->getTemplate(), $extraParams, $assignedVars );
    }
    public function getCategoryLevel2( $product )
    {
        foreach ( $product->getCategories() as $id ) {
            $category = new Category( $id );
            if ( $category->level_depth != 1 ) {
                if ( $category->level_depth > 2 ) {
                    foreach ( $category->getParentsCategories() as $category ) {
