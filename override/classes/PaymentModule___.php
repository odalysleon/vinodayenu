...
abstract class PaymentModule extends PaymentModuleCore
{
        ...
        $secure_key = false, Shop $shop = null)
    {
        ...
        $productFirst = new Product( $productsInCart[ 0 ][ 'id_product' ] );
        $category = $this->getCategoryLevel2($productFirst);
        $sata = _PS_OS_PREPARATION_;
        //$id_order_state = ($category != 'aceites')?$id_order_state:_PS_OS_PREPARATION_;
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            foreach ($package_list as $id_address => $packageByAddress) {
                foreach ($packageByAddress as $id_package => $package) {
                    ...
                    if(isset($_GET['envialia']))
                        $order->total_shipping_tax_incl = (float)$_GET['envialia_charge']+(float)$this->context->cart->getPackageShippingCost((int)$id_carrier, true, null, $order->product_list);
                    else
                    ...
                    if(isset($_GET['envialia']))
                        $order->total_paid = (float)$_GET['envialia_charge']+$order->total_paid_tax_incl;
                    else
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (isset($order->id)) {
                    ...
                    foreach ($order->product_list as $product) {
                        ...
                        $product_var_tpl['reference'] = $product_var_tpl['id_product'];
    ...
    private function getCategoryLevel2( $product )
    {
